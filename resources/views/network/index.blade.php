                    ...
                    <tr>
                        ...
                        <td>
                            <a href="#" class="btn btn-icon btn-outline-secondary edit-model-type"
                               ...
                               data-id="{{ $network->id }}"
                               data-value="{{ $network->name }}">
                                <span class="tf-icons bx bx-edit-alt"></span>
                            </a>
                            <form action="{{ route('network.destroy', array_merge(withLang(), ['id' => $network->id])) }}"
                                  method="POST"
                                  class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        ...
                                        onclick="return confirm('Are you sure you want to delete {{ $network->name }}?')">
                    ...
                    <form action="{{ route('network.store', withLang())}}" method="post">
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">{{__('network.locked.add')}}</h5>
            ...
            <div class="modal fade" id="editModelType" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <form action="{{ route('network.update', withLang())}}" method="post">
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">{{__('network.locked.edit')}}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="row">
